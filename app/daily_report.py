...
def render_daily_report(*, runs, now: datetime) -> EmailMessage:
    ...
    latest_run_age_minutes = _latest_run_age_minutes(runs, now)
    latest_run_age_label = (
        f"{latest_run_age_minutes} 分钟前" if latest_run_age_minutes is not None else "无记录"
    )
    expected_runs = 144
    run_health = _run_health_label(run_count, expected_runs, latest_run_age_minutes)
    alert_prefix = "⚠️" if run_health.startswith("告警") else "🐱"

    subject = (
        f"{alert_prefix}猫车雷达日报 {report_date}："
        f"运行{run_count}次，推送{notifications_sent}条"
    )
    text_body = f"""猫车雷达日报
            ...
            <tr>
              <td style="padding-bottom: 24px;">
                ...
                <p style="margin: 12px 0 0; font-size: 20px; line-height: 1.45;
                  color: #666;">
                  最近一次运行：{latest_run} · {latest_run_age_label} · 状态：{run_health}
                </p>
              </td>
            </tr>
...
def _latest_run_age_minutes(runs, now: datetime) -> int | None:
    if not runs:
        return None
    return max(0, int((now - runs[0].finished_at).total_seconds() // 60))


def _run_health_label(
    run_count: int,
    expected_runs: int,
    latest_run_age_minutes: int | None,
) -> str:
    if latest_run_age_minutes is None or latest_run_age_minutes > 30:
        return "告警：超过 30 分钟未运行"
